...
function shadowsocks_mysql($params) {
	...
	if (!$mysql) {
		$mysql = "Can not connect to MySQL Server".mysql_error()."Error";
	} else {
...
function shadowsocks_CreateNewPort($params) {
	...
	if (!$mysql) {
		$result = "Can not connect to MySQL Server".mysql_error()."Error";
	} else {
		mysql_select_db($params['configoption1'],$mysql);
...
function shadowsocks_CreateAccount($params) {
	...
	$port 				= shadowsocks_CreateNewPort($params);
  	...
  	if (!$mysql) {
  		$result = mysql_error()."Can not connect to MySQL Server".mysql_error()."Error".$params['serverip']."/".$params['serverusername']."/".$params['serverpassword'];
  	} else {
  		mysql_select_db($params['configoption1'],$mysql);
				...
				<table class=\"table table-bordered table-hover tc-table\">
					<tbody>
					  <tr>
						...
						<tr>
							<td>流量套餐</td><td>{$traffic} MB</td>
						</tr>
            <tr>
							<td>已经使用</td><td>{$Usage} MB</td>
						</tr>
						<tr>
							<td>剩余流量</td><td>{$Free} MB</td>
						</tr>
						<tr>
							<td>URL</td><td>{$sslink}</td>
						</tr>
						<tr>
							<td>QR Code</td><td>{$ssqr}</td>
						</tr>
					</tbody>
				</table>
				</div>
